...
class _MonthlySpendingCardState extends State<MonthlySpendingCard> {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider.value(
      value: _vm,
      child: Consumer<MonthlySpendingViewModel>(builder: (context, vm, _) {
        ...
        String bottomLabel(int index) {
          ...
          final s = DateFormat('MMM', 'es').format(DateTime(it.anio, it.mes, 1));
          return s[0].toUpperCase() + s.substring(1);
        }

        final last = hasData ? items.last : null;
        final pctChangeText = (last?.pctChangeFmt ?? '0.00%');
        final isUp = (last?.trend == 'sube' || last?.trend == 'sube_desde_cero');
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            if (!hasData || idx < 0 || idx >= items.length) {
